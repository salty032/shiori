...
const recorderApi: RecorderApi = {
  ...
  sendDone: (webm, duration, sessionId, drawnAt, diag) => {
  ...
  onBench: (cb: (data: BenchRequest) => void) => {
    ipcRenderer.on('recorder:bench', (_e, data) => cb(data))
  },
  sendBenchResult: (results) => {
