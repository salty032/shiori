...
export default forwardRef<HTMLDivElement, Props>(function Toolbar({
  ...
  // tag/site/type/プレフィックスの4種は排他表示なので、現在表示中のものを単一リストに正規化し、
  ...
  // tag:/site:/from:/to:/type: の演算子トークンを含まない、素のキーワード検索（空欄も含む）
  ...
  // tag:/site: も type: と同様、確定した値をテキストから消さずそのまま残す。
  ...
  function confirmPrefix(prefix: string): void {
    ...
    else if (prefix === 'type:') setForcedSuggestion({ prefix: 'type', search: next })
