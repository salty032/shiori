...
// 動画（録画クリップ・トリミング）の型・チャンネル。api.ts と分けてあるのはファイルの
// 大きさのためで、実際に公開される形は下の AppApi（ShioriApi と VideoApi の合成）。
...
export interface ClipGap {
   ...
   * その枚数に裏が取れているか。
   *
   * true は「合成へ送られたコマの累積数（rVFC の presentedFrames）と、時刻から出した推定が
   * 一致した」。false は推定だけで、**旧版の拡張で撮ったクリップは全部こちら**になる。
   *
   * **枚数が 1 違うと、そこから下のコマ番号が全部ずれる**（タイムシートは秒ではなくコマの
   * 通し番号を記録している）。推定を確定として出すと黙って誤らせるので、画面で分ける。
   */
  measured: boolean
  /**
   * 左右に残った絵のあいだだけにある、アニメとしての抜けコマ数（録画画像からの推定）。
   * `missing` は通知欠落数なので、2 コマ打ちの境界をまたぐとこの値より 1 大きくなりうる。
   * 無い場合は録画に境界が残っておらず、アニメのコマ数を推定できなかったことを表す。
   */
  animeMissing?: number
}
...
export interface ClipFrames {
  ...
  pts: number[]
  /**
   * 1 コマずつの長さ（秒）。**録画ファイル側の実測**で、そのコマの次に記録されている
   * ファイルのコマまでの差。
   *
   * **なぜ要るか。** コマを確実に表示させるシーク先は「そのコマの表示区間の中」でなければ
   * ならない。pts だけを持つ側（アプリ）は次の *行* までしか見えず、抜けをまたぐと行と行の
   * 間が広く空く。そこの中間を狙うと**抜けの中のコマに着地し、番号と違う絵が出る**
   * （id=297 の番号 323 で、13.404 と 13.559 の中間 13.4815 → ファイルのコマ 687 に着いていた）。
   * 長さはこちら側にしか無い（getVideoFramePts が返す全コマの時刻）ので、ここで渡す。
   *
   * 省略可。古い取得結果には無いので、無ければ従来どおり次の行までの中間で代用する。
   */
  dur?: number[]
  /**
   * 素材のコマ表に基づくか。
 ...
 *
 * VideoApi を ShioriApi へ宣言マージしない代わりに、**合成した型に名前を
