...
`shareExport` は `listImagesForExport()` を**無制限**に書き出す（キャプチャ全件）のに対し、
`shareImport` は metadata.jsonl を `MAX_EXPORT_IDS`（=1000）行で切り詰め、
**超過分は errors にも truncated にも記録されない**。1000 枚超のライブラリを
「書き出し → 読み込み」すると 1001 枚目以降が無言で消え、ユーザーは気づけない。
README の「ライブラリ全体（キャプチャ分）を書き出し／読み込み」という仕様と食い違う。

**修正**: 行数キャップを撤廃する（ループは元々 1 件ずつ逐次処理で、
`isValidCapturedAt` 等の防御は行単位に済んでいるため、上限の必然性は薄い）。
キャップを残すなら戻り値に `truncated: true` を追加し、SettingsModal の結果表示
（`shareImportStatus`）に「上限により一部は読み込まれていません」を出す。
...
### D-1. 【中】R-5（エクスポート並行実行ガード）がインポート側に未適用

**場所**: `app/src/main/ipc-share.ts:117`（shareImport）

作業ツリーの修正で `imagesExport` / `shareExport` には `isExporting` ガードが入ったが（R-5）、
`shareImport` には同型のガードがない。renderer 側のボタン disable だけが防壁で、
これは R-5 が「renderer 側ガードだけでは不十分」とした理屈と同じ穴。並行インポートが
起きると同じフォルダの画像が二重登録される（スマートフォルダは名前重複チェックで守られるが、
画像には重複チェックがない）。`if (isShareImporting) return { canceled: true }` の
同型ガードを 1 つ足して対称にする。`importFiles`（D&D取り込み）も同様だが、
こちらは重複登録の実害が小さいため任意。
...
インポートは最大 1000 件のファイルコピー + DB 登録をボタンラベルの「読み込み中...」
だけで待たせる（中止手段もない）。エクスポートと同じ `exportProgress` の枠組みに乗せるか、
少なくとも件数進捗をイベントで流すと対称になる。
...
**場所**: `app/src/main/ipc-share.ts:117-239`

1 行分の検証（basename 等価チェック・拡張子・captured_at クランプ・タグ正規化）が
120 行の IPC ハンドラに埋まっており、単体テストできない。ws-server.ts が
`parseExtensionMessage` を抽出してテスト済みなのと同じパターンで、
`parseShareEntry(line): ValidEntry | { error: string }` を純関数として切り出してテストする。
セキュリティ上の最重要検証（トラバーサル防止）がここにあるので、回帰網の価値が高い。
...
### DOC-2.【中】BLOG-DRAFT の AI タグの例が日本語になっている

**場所**: `BLOG-DRAFT.md:43`

「『屋外』『笑顔』『夜』といったタグが自動で付く」とあるが、WD Tagger の語彙は
Danbooru 系の**英語タグ**（`outdoors` / `smile` / `night`）で、日本語タグは付かない。
...
BLOG-DRAFT（「一括インポート」「エクスポート／インポート」）にも波及させ、
機能名 → 用語の対応表を 1 つ決めて全体を揃える。
